...
void Task::point_cloud_samplesTransformerCallback(const base::Time &ts, const ::base::samples::Pointcloud &point_cloud_samples_sample)
{
    ...
    this->tsdf_map->mergePointCloud(*sensor_point_cloud.get(), Eigen::Affine3d::Identity());
...
bool Task::configureHook()
{
    ...
    this->tsdf_map.reset(new ::maps::grid::TSDFVolumetricMap( ::maps::grid::Vector2ui(_number_cells.value().cast<unsigned int>()), _resolution.value()));
